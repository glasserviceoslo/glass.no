...
const DEFAULT_ZOOM_TRANSITON: AnimationOptionsWithValueOverrides<number> = {
  type: 'spring',
  damping: 23,
  mass: 0.85,
  stiffness: 100,
...
const DEFAULT_ROTATE_TRANSITON: AnimationOptionsWithValueOverrides<number> = {
  type: 'spring',
  damping: 23,
  mass: 0.85,
  stiffness: 100,
...
export class Camera {
  ...
  panTo(position: Vector, transition: AnimationOptionsWithValueOverrides<number> = DEFAULT_PAN_TRANSITON) {
    animate(this.motionValues.posX, position.x, transition);
    animate(this.motionValues.posY, position.y, transition);
  }

  setZoom(zoom: number, transition: AnimationOptionsWithValueOverrides<number> = DEFAULT_ZOOM_TRANSITON) {
    animate(this.motionValues.zoom, zoom, transition);
  }

  setRotation(rotation: number, transition: AnimationOptionsWithValueOverrides<number> = DEFAULT_ROTATE_TRANSITON) {
    animate(this.motionValues.rotation, rotation, transition);
  }

  follow(target: CameraTarget, transition: AnimationOptionsWithValueOverrides<number> = DEFAULT_PAN_TRANSITON) {
    if (this.following) {
